...
app.post('/selecttlanguage',(req,res)=>{
    ...
    //get name in student dashboard
    app.post('/getname', (req, res) => {
      const { email2 } = req.body;
      console.log(email2);
  
      const sql = 'SELECT uname FROM studentlogin WHERE email = ?';
      connection.query(sql, [email2], (error, result) => {
          if (error) {
              console.error('Error retrieving data from studentlogin table:', error);
              return res.status(500).json({ error: 'Database query failed' });
          }
  
         
              console.log(result[0].uname);
              // Send only the first result (assuming uname is unique)
              return res.json(result[0].uname);  // Send the first object which contains unam
